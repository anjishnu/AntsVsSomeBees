...
Name: Wenyu Zhu(person A) Anjishnu Kumar(person B)
Login:cs61a-dj cs61a-ej
TA:Allen Nguyen
Section:3:30-5:00
"""
...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            assert self.ant is None or self.ant.can_contain(insect) == True or insect.can_contain(self.ant) == True, 'Two ants in {0}'.format(self)
            if self.ant == None:
                self.ant = insect
                if insect.container==True:
                    insect.contain_ant(None)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        "*** YOUR CODE HERE ***"
        home, curr_pos =self.place, 0

        while(curr_pos < self.max_range):
            if home == None:
                return None
            if home.bees== []:
                home = home.entrance
                curr_pos+=1
            elif home.bees and home!=hive and curr_pos <= self.min_range:
                curr_pos +=1
                home = home.entrance
            
            elif home.bees and home!=hive and curr_pos > self.min_range:
                curr_pos +=1
                return random_or_none (home.bees)
            else:
                return None
...
def start_with_strategy(args, strategy):
    ...
    -i, --insane    Loads an insane assault plan. Good luck!False
    """
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if (self.wait_time ==0):
            ...
            count = 0
            if self.place.bees !=[]:
                some = random.randrange(0, len(self.place.bees))
                myBee = self.place.bees[some]
                ...
                some = 0
                while(flag):
                some = random.randrange(0, len(Bee.BeeDir))
                myBee= Bee.BeeDir[some]
                count+=1
                if myBee.armor!=0:
                    flag = False
            
                if (not isinstance(myBee.place, Hive)):"""
                self.eat_bee(myBee)
